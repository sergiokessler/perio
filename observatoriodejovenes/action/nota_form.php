...
$juventud_select = array('' => '-- Seleccione --');
$sql_sel = "select valor as k, valor as v from tabulado where clave = 'Juventud' order by valor";
$st = $db->query($sql_sel);
while ($row = $st->fetch(PDO::FETCH_ASSOC)) {
...
$form->addElement('select', 'new_row[tipo_de_nota]')
     ...
     ->addRule('required', 'Valor requerido')
     ;
$form->addElement('select', 'new_row[juventud]')
     ->setLabel('Juventud:')
