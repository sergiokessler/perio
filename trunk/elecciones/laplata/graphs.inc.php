...
//----------------------------------------------------------------------------------------------------
    ...
    var $titleFont = '';       // title font family: string (CSS specification)
    ...
    var $labelFont = '';       // label font family: string (CSS specification)
    ...
    var $absValuesFont = '';   // abs. values font family: string (CSS specification)
    ...
    var $percValuesFont = '';  // perc. values font family: string (CSS specification)
    ...
    var $legendFont = '';      // legend font family: string (CSS specification)
...
//----------------------------------------------------------------------------------------------------
    ...
    function create_hBar($value, $percent, $mPerc, $mPerc_neg, $max_neg, $mul, $valSpace, $bColor, $border, $spacer, $spacer_neg) {
      $bar = "<table border=0 cellspacing=0 cellpadding=0 height=100% width=100%><tr>";
    ...
    function create() {
      ...
      $graph = "<table border=0 cellspacing=0 cellpadding=$this->graphPadding width=$this->graphWidth><tr>";
      $graph .= '<td' . ($this->cssGRAPH ? ' style="' . $this->cssGRAPH . '"' : '') . '>';

      if($this->legend && $this->type != 'pbar' && $this->type != 'fader')
        $graph .= '<table border=0 cellspacing=0 cellpadding=0 width=100%><tr><td>';

      if($this->charts > 1) {
        $divide = ceil($bars / $this->charts);
        $graph .= '<table border=0 cellspacing=0 cellpadding=6 width=100%><tr valign=top><td>';
      }
      else $divide = 0;
      ...
      for($chart = $lcnt = 0; $chart < count($val); $chart++) {
        $graph .= '<table border=0 cellspacing=2 cellpadding=0 width=100%>';
      ...
      if($this->legend && $this->type != 'pbar' && $this->type != 'fader') {
        $graph .= '</td><td width=10>&nbsp;</td><td' . ($this->legendAlign ? ' valign=' . $this->legendAlign : '') . '>';
