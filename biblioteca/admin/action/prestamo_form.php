...
else
{
    ...
    $form->addDataSource(new HTML_QuickForm2_DataSource_Array(array(
        'new_row' => array('fecha_prestamo' => time()),
    )));
}
...
$inv = $form->addElement('text',   'new_row[inventario]', $campo_corto)
            ->setLabel('Inventario:')
            ->addClass('form-control')
            ;
$inv->addRule('required', 'Valor requerido');
$inv->addRule('regex', 'Debe ser un número', '/^\\d+$/');
